...
private fun diffLeafValues(
    ...
    if (before !is EstimationItem || after !is EstimationItem) return
    // Each leaf declares the ordered fields it wants diffed, so this stays
    // method-agnostic — branching on concrete leaf types here would make core
    // depend on the method modules. Iterating in `after` order and comparing
    // only the fields both sides declare reproduces the previous per-type
    // branches exactly whenever both leaves are the same type, which is the
    // only case the app can produce (an estimation's method is immutable).
    val beforeFields = before.diffFields().associateBy { it.name }
    for (field in after.diffFields()) {
        val old = beforeFields[field.name] ?: continue
        if (field.numeric) {
            cmpNum(path, field.name, old.number, field.number, out)
        } else {
            cmpStr(path, field.name, old.text, field.text, out)
        }
    }
}
...
private fun nodeTypeOf(node: EstimationNode): String = when (node) {
    ...
    // The leaf names its own discriminator; core does not know the leaf types.
    is EstimationItem -> node.nodeTypeLabel
}
